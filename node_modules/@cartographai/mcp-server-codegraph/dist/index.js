...
// src/index.ts
var BINARY_URL = "https://github.com/CartographAI/graph/releases/download/v0.1.0/graph";
var BINARY_PATH = "./bin/graph";
...
var directory = normalize(resolve(expandHome(args[0])));
    ...
    await new Promise((resolve2, reject) => {
      https.get(BINARY_URL, (response) => {
        if (response.statusCode === 302 || response.statusCode === 301) {
          ...
          https.get(redirectLocation, async (redirectedResponse) => {
          ...
          const fileStream = createWriteStream(BINARY_PATH);
          pipeline(response, fileStream).then(resolve2).catch(reject);
        }
      }).on("error", reject);
    });
    await chmod(BINARY_PATH, 493);
...
var ListFileEntitiesArgsSchema = z.object({
  path: z.string().describe("relative path of the file")
});
var ListEntityRelationshipsArgsSchema = z.object({
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: "Provides a list of all entities within a specified file. Returns a list of entities, including their names, types (e.g., Function, Class, Import), and locations (start and end lines). Use this to explore the contents of a file.",
        inputSchema: zodToJsonSchema(ListFileEntitiesArgsSchema)
      },
      {
        name: "list_entity_relationships",
        description: "Lists the relationships (Calls, Inheritance, Implementations) of a specify entity. Returns details about the related entities (source and target) and the type of relationship. Use this to understand how an entity interacts with other parts of the code.",
        inputSchema: zodToJsonSchema(ListEntityRelationshipsArgsSchema)
      }
    ]
  };
});
    ...
    switch (name) {
      case "index": {
        const { stdout, stderr } = await new Promise((resolve2, reject) => {
          exec(`${BINARY_PATH} ${directory} ${join(directory, "index.json")}`, (error, stdout2, stderr2) => {
            if (error) {
              reject({ error, stdout: stdout2, stderr: stderr2 });
            } else {
...
stderr: ${stderr}` }]
      ...
      case "list_file_entities": {
        const parsedArgs = ListFileEntitiesArgsSchema.safeParse(args2);
        if (!parsedArgs.success) {
        ...
        const fileNode = graph.nodes.find((node) => node.type === "File" && node.name === filename);
        if (!fileNode) {
          return {
        ...
        const getEntitiesRecursively = (nodeId, graph2) => {
          const node = graph2.nodes.find((n) => n.id === nodeId);
          if (!node) {
        ...
        const entities = fileNode.data.children.map((childId) => getEntitiesRecursively(childId, graph)).filter((entity) => entity.name);
        return {
          content: [{ type: "text", text: JSON.stringify(entities, null, 2) }]
        };
      }
      case "list_entity_relationships": {
        const parsedArgs = ListEntityRelationshipsArgsSchema.safeParse(args2);
        if (!parsedArgs.success) {
        ...
        const entityNode = graph.nodes.find((node) => {
        ...
        const relationships = graph.links.filter((link) => link.source_id === entityNode.id || link.target_id === entityNode.id).map((link) => {
          const sourceNode = graph.nodes.find((node) => node.id === link.source_id);
          const targetNode = graph.nodes.find((node) => node.id === link.target_id);
          const sourceFileNode = graph.nodes.find((node) => node.id === sourceNode.file_id);
          const targetFileNode = graph.nodes.find((node) => node.id === targetNode.file_id);
