...
func generatePDF(cfg *types.ReportConfig, reportID, logRoot string, scanEntries []ScanEntry, appearance *types.ReportAppearance) (string, error) {
	...
	// Use positive Y (A4 height 297mm - 12mm = 285mm) instead of negative Y.
	// SetY(-12) causes gofpdf to truncate all pages after the current one,
	// reducing a 300-page PDF to 1 page. This is the root cause of the
	// "report only generates 1 page" bug.
	if *appearance.ShowHeader {
		...
		for i := 1; i <= totalPages; i++ {
			pdf.SetPage(i)
			pdf.SetY(285) // A4 height (297mm) - 12mm = 285mm from top
			pdf.SetFont("Helvetica", "", 8)
			pdf.SetTextColor(150, 150, 150)
			pdf.Cell(0, 5, fmt.Sprintf("Page %d/%d — Generated by LOGReport", i, totalPages))
			pdf.SetTextColor(0, 0, 0)
		}
