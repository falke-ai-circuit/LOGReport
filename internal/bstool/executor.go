...
type executor interface {
	// execute runs the command and returns stdout bytes, exit code, and any error.
	execute(ctx context.Context, exePath string, args []string, env []string) (stdout []byte, exitCode int, err error)
}
