...
**Changes Made**:
- Updated [`LIS_TOKEN_PATTERN`](src/commander/presenters/node_tree_presenter.py:line) regex to `r"^([\w-]+)_[\d-]+_(.+)\.lis$"`
- Enhanced pattern to correctly match LIS filenames with node identifiers and timestamps

**Root Cause**:
- Previous regex pattern did not properly capture LIS filename structure, causing some files to be excluded from listings

**Impact**:
- Correct display of all LIS log files in the application
- Improved consistency with FBC/RPC/LOG file handling patterns
- Resolved user-reported issue with missing LIS files in node tree view
