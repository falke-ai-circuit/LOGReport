...
@dataclass
class CellValueAppeared:
    """Represents a cell where value appeared in live data (was empty/N/E in file)"""
    row: int
    col: int
    file_value: str
    live_value: str
...
class ComparisonResult:
    ...
    differences: List[CellDifference]  # List of cell differences (file had value -> live different/missing) - RED
    value_appeared: List[CellValueAppeared]  # List of cells where value appeared (file empty/N/E -> live has value) - YELLOW
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'value_appeared': [
                (v.row, v.col, v.file_value, v.live_value)
                for v in self.value_appeared
            ],
            'errors': [
...
class FbcComparisonService:
    ...
    def _is_empty_value(self, value: str) -> bool:
        """
        Check if a value is considered empty (empty string, 'N/E', 'N', or whitespace).
        
        Args:
            value: Cell value to check
            
        Returns:
            True if value is empty/N/E, False otherwise
        """
        if not value:
            return True
        normalized = str(value).strip().upper()
        return normalized in ('', 'N/E', 'N', 'NOT EXISTS')
    ...
    def _compare_tables(self, file_data: FbcTableData, live_data: FbcTableData) -> ComparisonResult:
        ...
        Color logic:
        - GREEN: Values match
        - RED: File had a value -> Live is different or missing (actual problem)
        - YELLOW: File was empty/N/E -> Live has a value (new data appeared)
        ...
        value_appeared: List[CellValueAppeared] = []
        ...
        for row_idx, file_row in enumerate(file_data.rows):
            ...
            for col_idx, file_header in enumerate(file_data.headers):
                ...
                if file_val_norm == live_val_norm:
                    ...
                    # Check if file value was empty/N/E and live value has data
                    file_was_empty = self._is_empty_value(file_value)
                    live_has_value = not self._is_empty_value(live_value)
                    
                    if file_was_empty and live_has_value:
                        # YELLOW: File was empty/N/E -> Live has value (new data appeared)
                        self.logger.debug(f"VALUE_APPEARED at row {row_idx} display_col {display_col_idx} ({file_header}): file='{file_val_norm}' (empty) -> live='{live_val_norm}' (has value)")
                        value_appeared.append(CellValueAppeared(
                            row=row_idx,
                            col=display_col_idx,
                        ...
                        # RED: File had value -> Live is different or missing (actual problem)
        ...
        self.logger.info(
            f"Comparison complete: {len(matches)} matches, {len(differences)} differences (RED), "
            f"{len(value_appeared)} value_appeared (YELLOW), {len(errors)} errors, {match_percentage:.1f}% match"
        )
